    ...
    public partial class Form1 : Form
    {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                //System.IO.File.WriteAllText("D:\\test.txt", binaryFull);
                ...
                */
                ...
                Color[] allPixels = getPixels(img, ARGB, pixelsUsed);
                Dictionary<int, byte> changes = new Dictionary<int, byte>();
                int ln = 0;
                for(int i = seed[0] - 1; i <= pixelsUsed; i += seed[ln % seed.Length])
                {
                    int[] color = new int[] { allPixels[i].A, 
                        allPixels[i].R, allPixels[i].G, allPixels[i].B };
                    int argb = color[ARGB];
                    string pixelBinary = Convert.ToString(argb, 2).PadLeft(8, '0').Remove(8 - lsb, lsb);

                    b = Convert.ToInt32(pixelBinary + binary[ln], 2);

                    if (b != argb) { changes.Add(i - 1, (byte)argb); }
                    color[ARGB] = b;
                    allPixels[i] = Color.FromArgb(color[0], color[1], color[2], color[3]);
                    if (ln++ >= binary.Length - 1)
                        break;
                }
                img = setPixels(img, allPixels);
        ...
        Color[] getPixels(Bitmap img, int ARGB, int size)
        {
        ...
        private Bitmap setPixels(Bitmap img, Color[] pixels)
        {
